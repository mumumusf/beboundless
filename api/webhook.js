...
// 从环境变量获取Token
const BOT_TOKEN = process.env.BOT_TOKEN || '8185015603:AAE26ZwNvlPfp-24_rLhGygkgid-Ft80Nkc';

// 存储在内存中的用户状态（注意：Vercel函数重启时会丢失）
    ...
    try {
        ...
        if (update.message) {
            ...
            if (text?.startsWith('/')) {
                if (text === '/start') {
                    ...
                    await sendMessage(chatId, getText(chatId, 'welcome'));
                } else if (text === '/help') {
                    await sendMessage(chatId, getText(chatId, 'help'));
                    ...
                    await sendMessage(chatId, getText(chatId, 'selectLanguage'), keyboard);
                } else if (text === '/query') {
                    const currentState = userStates.get(chatId);
                    ...
                    await sendMessage(chatId, getText(chatId, 'selectNetwork'), keyboard);
                    ...
                    await sendMessage(chatId, getText(chatId, 'analyzeWelcome') + '\n\n' + getText(chatId, 'selectAnalyzeMode'), keyboard);
                }
            } else {
                // 处理普通消息
                const userState = userStates.get(chatId);
                ...
                if (!userState || !userState.step) {
                    await sendMessage(chatId, getText(chatId, 'useQueryFirst'));
                ...
                if (userState.step === 'waitingForAddress') {
                    ...
                    if (!isValidAddress(address)) {
                        await sendMessage(chatId, getText(chatId, 'invalidAddress'));
                        return res.status(200).json({ ok: true });
                    }
                    
                    // 显示查询中的消息
                    const loadingResponse = await sendMessage(chatId, getText(chatId, 'querying'));
                    ...
                    try {
                        ...
                        // 编辑加载消息为结果
                        await editMessage(chatId, loadingResponse.result.message_id, formattedResult);
                        ...
                        // 提供继续查询的选项
                        setTimeout(async () => {
                            await sendMessage(chatId, getText(chatId, 'continueQuery'));
                        }, 1000);
                        
                    } catch (error) {
                        // 编辑加载消息为错误信息
                        await editMessage(chatId, loadingResponse.result.message_id, 
                            `${getText(chatId, 'queryError')} ${error.message}`);
                    ...
                    if (!orderId || orderId.length < 10) {
                        await sendMessage(chatId, getText(chatId, 'invalidOrderId'));
                        return res.status(200).json({ ok: true });
                    }
                    
                    // 显示分析中的消息
                    const loadingResponse = await sendMessage(chatId, getText(chatId, 'analyzingOrder'));
                    ...
                    try {
                        ...
                        // 编辑加载消息为错误信息
                        await editMessage(chatId, loadingResponse.result.message_id, 
                            `${getText(chatId, 'analyzeError')} ${error.message}`);
                    ...
                    if (!cycles || cycles <= 0) {
                        await sendMessage(chatId, getText(chatId, 'enterProgramCycles'));
                        return res.status(200).json({ ok: true });
                    }
                    ...
                    await sendMessage(chatId, getText(chatId, 'enterDeliveryTime'));
                    ...
                    await sendMessage(chatId, getText(chatId, 'enterEthPerMC'));
        ...
        if (update.callback_query) {
            ...
            if (data.startsWith('lang_')) {
                ...
                await editMessage(chatId, messageId, getText(chatId, 'languageChanged'));
                ...
                if (data === 'analyze_by_id') {
                    ...
                    await editMessage(chatId, messageId, getText(chatId, 'enterOrderId'));
                    ...
                    await editMessage(chatId, messageId, getText(chatId, 'enterProgramCycles'));
                }
            }
        }
